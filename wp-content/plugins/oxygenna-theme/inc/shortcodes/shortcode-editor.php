...
$shortcode_options = include_once OXY_FRAMEWORK_DIR . 'shortcodes/shortcode-options.php';
...
$preview_url = get_home_url( $blog_id, 'wp-admin/admin-ajax.php?_ajax_nonce=' . $nonce . '&action=oxy_shortcode_preview&sc=', $scheme );
?>
    ...
    <head>
        ...
        <script type="text/javascript" charset="utf-8">
            ...
            jQuery(document).ready( function() {
                ...
                jQuery( '.slider-option,.font-option' ).change( throttledUpdatePreview );
                // check for font-variant updates
                jQuery( 'body' ).bind('fontChange', throttledUpdatePreview );
                // check for colour changes
                jQuery( '.colour-option' ).bind('colourChange', throttledUpdatePreview );
