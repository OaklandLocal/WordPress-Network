...
class OxyQuickUpload {
    ...
    function __construct() {
        if( is_admin() ) {
            ...
            if( file_exists( $uploaders_file ) ) {
                $this->_uploaders = include $uploaders_file;
            }
            else {
        ...
        if( $this->_uploaders !== null && isset( $_GET['page'] ) ) {
            foreach( $this->_uploaders as $post_type => $options ) {
                if( $_GET['page'] == $post_type . '-quick' ) {
                    wp_enqueue_style( 'oxy-quick-uploader', OXY_FRAMEWORK_URI . 'css/quick-uploader/quick-uploader.css' );
                }
            }
        }
    }
    ...
    function select_images_page( $post_type, $options ) {
        ...
        if (!current_user_can('upload_files'))
            wp_die(__('You do not have permission to upload files.', 'PLUGIN_TD'));
        ...
        if ( isset($action) && $action == 'edit' && !$ID )
            wp_die(__('You are not allowed to be here', 'PLUGIN_TD'));
        ...
        $title = __('Upload New Media', 'PLUGIN_TD');
        ...
        <?php submit_button( __( 'Use these images', 'PLUGIN_TD' ), 'button savebutton', 'save' ); ?>
        </form>
        </div>
...
?>
                   ...
                   <?php
                    ...
                    <div id="post-body" class="metabox-holder columns-2">
                        <div id="post-body-content">
                            ...
                            <?php submit_button( __('Save', 'PLUGIN_TD') . ' ' .$options['item_plural'] ); ?>
                        </div>
                    </div>
                </div>
            </form>
    ...
    function build( $post_type, $options ) {
        ...
        echo '<div id="message" class="updated fade"><p><strong>' . sprintf(__('Created %d %s', 'PLUGIN_TD'), count( $_POST['media_attachment_id'] ), $options['item_plural'] ) . '</strong></p></div>';
    }
}
