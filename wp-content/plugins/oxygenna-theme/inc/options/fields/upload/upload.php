...
class OxyUpload extends OxyOption {
    ...
    private function create_option( $value , $url ) {
        ...
        $option .= '<input type="button" class="oxy-set-image" value="' . __( 'Set Image', 'PLUGIN_TD' ) . '" style="' . $hide_set_button . '"/>';
        $option .= '<input type="button" class="oxy-remove-image" value="' . __( 'Remove Image', 'PLUGIN_TD' ) . '" style="' . $hide_preview . '"/>';
    ...
    public function enqueue() {
        parent::enqueue();
        // load styles
        wp_enqueue_style( 'oxy-option-upload', OXY_FRAMEWORK_URI . 'css/options/oxy-option-upload.css', array( 'thickbox' ) );
        // load scripts
        wp_enqueue_script( 'upload-field', OXY_FRAMEWORK_URI . 'options/fields/upload/upload.js', array( 'jquery', 'thickbox', 'media-upload' ) );
    }
}