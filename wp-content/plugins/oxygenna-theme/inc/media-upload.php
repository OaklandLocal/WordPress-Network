...
class OxyMediaUpload {
    function __construct() {
        if( isset( $_REQUEST['oxy_upload_to_option'] ) ) {
            ...
            wp_enqueue_script( 'theme-options-upload', OXY_FRAMEWORK_URI . 'javascripts/theme-options-upload.js' );
        }
    }
    ...
    function image_attachments( $form_fields, $post ) {
        ...
        $send = '<input type="submit" onclick="oxyThemeOptions.mediaOptionSelectImage(\'' . $_REQUEST['input_id'] . '\',\'' . $post->guid . '\',\'' . $post->ID . '\')" class="button" value="' .__('Select Image', 'PLUGIN_TD') . '">';
