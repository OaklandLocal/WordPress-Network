...
class OxyMetabox {
    ...
    function add_meta_to_taxonomies(){
    ...
    function admin_enqueue_scripts() {
        ...
        if ( isset($screen) && 'post' == $screen->base &&  isset($this->options['pages']) &&  in_array( $screen->post_type, $this->options['pages'] ) || isset($screen) && 'edit-tags' == $screen->base && isset($this->options['taxonomies']) &&  in_array( $screen->taxonomy, $this->options['taxonomies'] )  ){
            ...
            wp_enqueue_style( 'oxy-metabox-global', OXY_FRAMEWORK_URI . 'css/metaboxes/metabox-global.css', array( 'jquery-oxygenna-ui-theme' ) );
            wp_enqueue_script( 'metabox-options-global', OXY_FRAMEWORK_URI . 'javascripts/metabox-options-global.js', array( 'jquery', 'jquery-ui-tooltip' ) );
     ...
     * @return void
     **/
    function render_taxonomy_metabox( $term ){
        foreach( $this->options['fields'] as $field ) {
            $id = THEME_SHORT . '_' . $field['id'];
            $attr = array( 'name' => THEME_SHORT . '-options[' . $id . ']' );
            // we are in edit screen
            if(is_object($term)){
                $value = get_option( THEME_SHORT . '-tax-mtb-' .$field['id']. $term->term_id, '' ) == false ? '' : get_option( THEME_SHORT . '-tax-mtb-' . $field['id'].$term->term_id, '' );
            }
            else{
    ...
    function save_meta_to_taxonomies( $term_id, $tt_id ){
        foreach (  $this->options['fields'] as $field ) {
            $id = THEME_SHORT . '_' . $field['id'];
