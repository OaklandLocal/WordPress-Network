...
function siiimple_google_fonts(){
	...
	wp_enqueue_style( 'Raleway', 'http://fonts.googleapis.com/css?family=Raleway:400,200,700' );
    
	wp_enqueue_style( 'SansPro', 'http://fonts.googleapis.com/css?family=Source+Sans+Pro' );
			...
			<footer class="comment-meta">
				<div class="comment-author vcard">
					<?php
						...
						printf( __( '%1$s on %2$s <span class="says">said:</span>', 'siiimple' ),
							sprintf( '<span class="fn">%s</span>', get_comment_author_link() ),
							sprintf( '<a href="%1$s"><time pubdate datetime="%2$s">%3$s</time></a>',
								esc_url( get_comment_link( $comment->comment_ID ) ),
								get_comment_time( 'c' ),
								/* translators: 1: date, 2: time */
								sprintf( __( '%1$s at %2$s', 'siiimple' ), get_comment_date(), get_comment_time() )
							)
						);
					?>
					
					<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( 'Reply <span>&darr;</span>', 'siiimple' ), 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->

				<?php edit_comment_link( __( 'Edit', 'siiimple' ), '<span class="edit-link">', '</span>' ); ?>
