...
<form action="<?php echo esc_url( $product->add_to_cart_url() ); ?>" class="cart" method="post" enctype='multipart/form-data'>
	<table class="table table-striped" cellspacing="0" class="group_table">
		<tbody>
				...
				<tr>
					<td>
						...
						<?php elseif ( ! $quantites_required ) : ?>
							...
							<a href="<?php echo esc_url( $child_product['product']->add_to_cart_url() ); ?>" rel="nofollow" class="single_add_to_cart_button button alt btn btn-primary add_to_cart_button"><?php echo apply_filters( 'single_add_to_cart_text', __( 'Add to cart', 'woocommerce' ), $child_product['product']->product_type ); ?></a>
					...
					<td><p itemprop="price" class="price"><?php echo $child_product['product']->get_price_html(); ?></p>
	...
	<?php if ( $quantites_required ) : ?>
		...
		<button type="submit" class="single_add_to_cart_button button alt btn btn-primary add_to_cart_bu add_to_cart_buttontton"><?php echo apply_filters('single_add_to_cart_text', __( 'Add to cart', 'woocommerce' ), $product->product_type); ?></button>
